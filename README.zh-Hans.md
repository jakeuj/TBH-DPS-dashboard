...
![游戏内叠加面板 — DPS、承受伤害与关卡比较三面板](image/TaskBarHero_MS3KA9KGlR.jpg)
...
## 关卡比较（F11）
按 **F11** 打开**关卡比较面板**：把你存下的记录按关卡分组，拿当前这场跟**基准**（默认最快通关，或你手动钉选的一场）对照，
显示时长、**有效输出 vs 停输出（跑路）时间**、平均/峰值/暴击、伤害分配、**每波时间**，以及**装备与技能变更**
——让你看出「换了配置后通关效率差多少」。用 ◀ ▶ 翻看场次、≪ ≫ 切换关卡、钉选钮设定基准。

关卡名称、角色分页与技能名称**跟随游戏语言**——游戏切成 English / 日本語 / 繁體中文 / 简体中文 / Español，面板会**实时切换、不用重启**。

<img src="image/TaskBarHero_5BRF6aiQF5.png" alt="关卡比较面板" width="420">

> *上方通关秒数趋势折线（点某点看该场详细）；下方「基准 ∣ 这场」对齐两栏、绿/红标出变化，并列出技能变更。*
