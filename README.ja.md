...
### B. macOS CrossOver のチェック／修復
CrossOver では、Wine が BepInEx の proxy ではなく内蔵の `winhttp.dll` を読み込む場合があります。また、macOS がダウンロードしたファイルに quarantine を付けることもあります。ゲーム内に overlay が出ず、`<ゲームフォルダ>\BepInEx\LogOutput.log` も作成されない場合は、repo ルートでまず確認します：

```sh
bash scripts/repair-crossover-macos.sh --check
```

`winhttp` override の不足、または quarantine marker が報告された場合は、修復して Steam から再起動します：

```sh
bash scripts/repair-crossover-macos.sh --repair --launch
```

通常、bottle を指定する必要はありません。スクリプトは `TaskBarHero.exe` を含む CrossOver bottle を自動で探します。CrossOver bottle が複数ある場合は `--bottle "<your-bottle-name>"` を追加してください。Steam library が標準位置でない場合は `--game-dir "/path/to/TaskbarHero"` を追加してください。`--check` は読み取り専用です。`--repair` は macOS quarantine metadata の削除と、`TaskBarHero.exe` 用の scoped Wine override 設定だけを行い、ゲームの数値やプラグインコードは変更しません。
